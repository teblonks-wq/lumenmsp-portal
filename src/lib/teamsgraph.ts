...
import { saveBufferAttachment } from './attachments';
...
// Pictures pasted/attached in a Teams chat arrive as hostedContents — Graph URLs inside the
// message HTML that need a bearer token, so a browser can NEVER load them directly (this is why
// Teams pictures showed as nothing in the Portal). Download each one with our token, store it in
// static/attachments, and return the local public URLs to embed in the stored message instead.
async function saveTeamsPictures(chatId: string, messageId: string, token: string): Promise<{ name: string; url: string }[]> {
  const out: { name: string; url: string }[] = [];
  const base = `/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(messageId)}`;
  const hc = await graphGet(base + '/hostedContents', token);
  let i = 0;
  for (const h of (hc.value || [])) {
    try {
      const res = await fetch(`${GRAPH}${base}/hostedContents/${encodeURIComponent(String(h.id))}/$value`, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) { console.error('[teamsgraph] hostedContent download failed:', res.status); continue; }
      const ct = res.headers.get('content-type') || '';
      if (ct && !ct.startsWith('image/')) continue; // pictures only
      const ext = ct.includes('jpeg') ? 'jpg' : ct.includes('gif') ? 'gif' : ct.includes('webp') ? 'webp' : 'png';
      const buf = Buffer.from(await res.arrayBuffer());
      if (!buf.length) continue;
      out.push(saveBufferAttachment(buf, `teams-picture-${++i}.${ext}`));
    } catch (e) { console.error('[teamsgraph] hostedContent download failed:', (e as Error).message); }
  }
  return out;
}
...
async function processChatMessage(chatId: string, lp: any, selfUserId: string): Promise<void> {
  ...
  // The poller only carries lastMessagePreview — fetch the FULL message so long texts aren't
  // truncated and pictures (hostedContents) can be pulled down. Best-effort: if Graph hiccups,
  // fall back to the preview body rather than dropping the message.
  let fullHtml = lp.body?.content || '';
  let pictures: { name: string; url: string }[] = [];
  try {
    const token = await getDelegatedToken();
    const full = await graphGet(`/chats/${encodeURIComponent(chatId)}/messages/${encodeURIComponent(String(lp.id))}`, token);
    if (full?.body?.content) fullHtml = full.body.content;
    pictures = await saveTeamsPictures(chatId, String(lp.id), token);
  } catch (e) { console.error('[teamsgraph] full message/pictures fetch failed:', (e as Error).message); }

  const text = htmlToPlain(fullHtml) || (pictures.length ? `[${pictures.length === 1 ? 'picture' : pictures.length + ' pictures'}]` : '[message]');
  ...
  let bodyHtml = '<div style="white-space:pre-wrap;">' + escHtml(text) + '</div>';
  if (pictures.length) {
    bodyHtml += '<div style="margin-top:8px;">' + pictures.map((p) =>
      `<a href="${p.url}" target="_blank" rel="noopener"><img src="${p.url}" alt="Picture from Teams" style="max-width:340px;max-height:260px;border-radius:8px;display:block;margin-top:6px;"></a>`
    ).join('') + '</div>';
  }
  await pool.query(
    ...
    [ticketId, senderName, senderEmail || null, null, text, bodyHtml, msgId]
  );
