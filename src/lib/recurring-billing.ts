...
export async function remindCommsBillRun(force = false): Promise<number> {
  ...
  // Completing a run immediately rolls the OPEN period forward, so the check above then looks
  // at the freshly opened period — whose done-marker can never exist yet. That kept the 4-hourly
  // nags firing on the 22nd/23rd even though the month was already billed (seen 2026-07-22, the
  // day after July was completed). A run completed at ANY point during the current calendar month
  // has satisfied this month's 20th->23rd window, so stay quiet regardless of which period is open.
  if (!force) {
    const done = await pool.query('SELECT value FROM settings WHERE "group"=$1 AND key LIKE $2', ['comms', 'billrun_done_%']);
    const now = new Date();
    const doneThisMonth = done.rows.some((r: any) => {
      const d = new Date(r.value);
      return !isNaN(d.getTime()) && d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth();
    });
    if (doneThisMonth) { return 0; }
  }
