...
// Per-customer breakdown for the compose page: which customers are represented,
// and which contribute nobody (no default domain / no contacts on that domain /
// no contacts with an email at all). Surfacing this stops the list "feeling small"
// for invisible reasons — the fixes live in Customers → Data missing.
export interface CoverageCustomer {
  id: number; name: string; domain: string | null;
  contacts_with_email: number; on_domain: number; opted_out: number;
}
export interface Coverage {
  totalCustomers: number;
  represented: CoverageCustomer[];   // ≥1 sendable on-domain contact
  noDomain: CoverageCustomer[];      // no default domain set → nobody qualifies
  noMatch: CoverageCustomer[];       // has domain + emails, but none ON the domain
  noContacts: CoverageCustomer[];    // no contacts with an email address
...
export async function audienceCoverage(statusFilter: 'active' | 'all'): Promise<Coverage> {
  ...
  const { rows } = await pool.query(
    `SELECT c.id, c.name, c.domain,
            COUNT(cc.id) FILTER (WHERE cc.email IS NOT NULL AND cc.email <> '' AND cc.archived = false)::int AS contacts_with_email,
            COUNT(cc.id) FILTER (WHERE cc.email IS NOT NULL AND cc.email <> '' AND cc.archived = false
              AND c.domain IS NOT NULL AND lower(split_part(cc.email,'@',2)) = lower(c.domain)
              AND NOT COALESCE(cc.marketing_opt_out, false))::int AS on_domain,
            COUNT(cc.id) FILTER (WHERE cc.email IS NOT NULL AND cc.email <> '' AND cc.archived = false
              AND c.domain IS NOT NULL AND lower(split_part(cc.email,'@',2)) = lower(c.domain)
              AND COALESCE(cc.marketing_opt_out, false))::int AS opted_out
     FROM customers c
     LEFT JOIN customer_contacts cc ON cc.customer_id = c.id
     WHERE c.deleted_at IS NULL AND c.is_placeholder = false ${where}
     GROUP BY c.id, c.name, c.domain
     ORDER BY c.name ASC`,
    []
  );
  const cov: Coverage = { totalCustomers: rows.length, represented: [], noDomain: [], noMatch: [], noContacts: [], optedOutTotal: 0 };
  for (const r of rows as CoverageCustomer[]) {
    cov.optedOutTotal += r.opted_out;
    if (r.on_domain > 0) cov.represented.push(r);
    else if (!r.domain) cov.noDomain.push(r);
    else if (r.contacts_with_email > 0) cov.noMatch.push(r);
    else cov.noContacts.push(r);
  }
  return cov;
}
...
// Campaigns carry the SAME signature block as support emails (custom Branding
// signature if saved — logo included — else the default contact details + banner
// + disclaimer), under a company-wide name rather than a personal one. Order:
// body → signature → unsubscribe line.
export const MAIL_SIGNATURE_NAME = 'The Lumen IT Solutions Team';

export async function campaignSignatureHtml(): Promise<string> {
