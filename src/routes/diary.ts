...
import {
  DIARY_KINDS, isDiaryKind, dayKeyOf, mondayOf, addDays, diaryPeople, loadWeek, saveEntry,
  findClashes, findAllDayClashes, diaryWhenText, Clash,
} from '../lib/diary';
...
async function checkAndSave(req: Request, res: Response, id: number | null) {
  ...
  else if (inp.kind === 'timeoff' && inp.dayKey) clashes = await findAllDayClashes(inp.personIds, inp.dayKey, id);
  ...
  const result = await saveEntry(id, inp);
  ...
  pushEntry(result.id!).catch(() => {});   // mirror to M365 in the background
  res.json({ ok: true, id: result.id, warning });
}
...
router.post('/diary/entries/:id/status', requireAuth, async (req: Request, res: Response) => {
  ...
  const row = (await pool.query(`UPDATE diary_entries SET status=$1, updated_at=NOW() WHERE id=$2 RETURNING title`, [status, id])).rows[0];
  if (!row) { res.redirect(back + '?err=' + encodeURIComponent('That entry is gone.')); return; }
  await logActivity(user.id, 'diary_' + status, 'diary_entries', id, `${row.title} → ${status}`);
  if (status === 'cancelled') removeEntry(id).catch(() => {});
  else pushEntry(id).catch(() => {});
  res.redirect(back + '?msg=' + encodeURIComponent(status === 'done' ? 'Marked done.' : status === 'cancelled' ? 'Cancelled — its calendar copies are being removed.' : 'Rebooked.'));
});
...
router.post('/tickets/:id/promise', requireAuth, async (req: Request, res: Response) => {
  ...
  const r = await saveEntry(null, {
    ...
    personIds: [user.id], startEpoch: null, endEpoch: null, dayKey, bufferMins: 0, createdBy: user.id,
  });
