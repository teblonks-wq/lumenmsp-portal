...
// Lives in its own file since 21 Aug: this used to be routes/network.ts, which network
// DISCOVERY (printers, switches, SNMP) has taken over. Same URLs, same behaviour - the
// footer on every page polls /n3twrx/alerts.json, so these paths must not move.
const router = Router();
...
router.get('/n3twrx', requireAuth, async (req: Request, res: Response) => {
  ...
  res.render('n3twrx/index', { user: req.session.user!, alerts, status, notice: req.query.msg || null });
});
